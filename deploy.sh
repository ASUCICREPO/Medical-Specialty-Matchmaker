...
if ! aws secretsmanager describe-secret --secret-id "$GITHUB_SECRET_NAME" --region "$AWS_REGION" &> /dev/null; then
  ...
  echo ""
  echo "To enable automatic Amplify deployments from GitHub, create a Personal Access Token:"
  echo "  1. Go to GitHub Settings → Developer settings → Personal access tokens"
  echo "  2. Generate new token with 'repo' permissions"
  echo "  3. Run: aws secretsmanager create-secret --name github-token --secret-string 'YOUR_TOKEN' --region $AWS_REGION"
  echo ""
  read -p "Do you want to continue without GitHub integration? (y/n) " -n 1 -r
  echo
  if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    print_error "Deployment cancelled. Please set up GitHub token first."
  fi
  print_warning "Continuing without GitHub integration..."
else
...
print_status "Phase 6: Configuring Frontend Environment..."
...
print_status "Created .env file with API endpoints"
